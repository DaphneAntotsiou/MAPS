'''
This script is heavily based on @metaworld/envs.mujoco.sawyer_xyz.v2.sawyer_peg_insertion_side_v2.
...
class SawyerPegInsertionSideEnvV2(SawyerXYZEnv):
    ...
    ################### STATE SETTER EXTENSION #######################
    def ss(self, state_dict):
        super().ss(state_dict)
        self.peg_head_pos_init = self._get_site_pos('pegHead')
        self.sim.model.body_pos[self.model.body_name2id('box')] = self._target_pos - np.array([.03, .0, .13])