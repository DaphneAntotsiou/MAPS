'''
This script is heavily based on @metaworld/envs.mujoco.sawyer_xyz.sawyer_xyz_env.
It extends it with setting random for all environments and adds a getter and setter of state
'''
import abc
import copy
import pickle
...
from metaworld.envs.mujoco.sawyer_xyz.sawyer_xyz_env import SawyerMocapBase
...
class SawyerXYZEnv(SawyerMocapBase, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        self._freeze_rand_vec = False   ################################### ALWAYS RANDOM INIT #########################
        ...
        self._set_task_called = True    ###################### SET TASK ###################################
...
########################################## NEW EXTENSION #######################################
    def ss(self, state_dict):
        self.curr_path_length = state_dict.get('curr_path_length', 0)
        self._target_pos = state_dict['target_pos'].copy()
        # self._set_goal_marker(self._state_goal)
        self.obj_init_pos = state_dict['obj_init_pos']
        self.data.set_mocap_pos('mocap', state_dict['mocap_pos'])
        self.data.set_mocap_quat('mocap', state_dict['mocap_quat'])
    ...
    def gs(self):
        return {
                  ...
                  'obj_init_pos': self.obj_init_pos.copy(),
                  'curr_path_length': self.curr_path_length
        }
