'''
This script is heavily based on @openai/baselines.logger
'''
...
class HumanOutputFormat(KVWriter, SeqWriter):
    def __init__(self, filename_or_file, append=False):
        if isinstance(filename_or_file, str):
            self.file = open(filename_or_file, 'wt') if not append or not osp.isfile(filename_or_file) \
                else open(filename_or_file, 'a+t')
            self.own_file = True
        else:
...
class CSVOutputFormat(KVWriter):
    def __init__(self, filename, append=False):
        self.file = open(filename, 'w+t') if not append or not osp.isfile(filename) else open(filename, 'a+t')
        self.keys = []
        self.sep = ','
