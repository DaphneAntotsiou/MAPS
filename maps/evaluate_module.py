...
def argsparser():
    parser = argparse.ArgumentParser("Imitation Multi Task Learning with module Dictionary evaluation")
    ...
    parser.add_argument('--load_model_path', help='model directory', default=
    "checkpoint/tasks_10_traj_20_short_onehot-20_10_3layer_32bs_1.0_0.1_0.5_1.0_0.75/1/30000")
    boolean_flag(parser, 'eval_hours', default=False, help='evaluate all the hours of a train run')
...
def init_model(envs, dir, find_checkpoint=True, d_policy=None, module=-1):
    ...
    if not d_policy:
        ...
        selector_policy = SelectorPolicy("router", module_num=module_num, task_space=taskid_space,
                                      ob_space=list(envs.values())[0]['env'].observation_space,
                                      hidden_size=r_size, loss=False, task_num=10)
        if module > -1:
            module_id = [0 for _ in range(module_num)]
            module_id[module] = 1
            selector_policy.fix_output_module(module_id)
        print("creating modular policy")
        d_policy = ModularPolicy("d_pi", selector_policy=selector_policy, module_num=module_num,
                                             ob_space=list(envs.values())[0]['env'].observation_space,
...
def runner(envs, load_model_path, timesteps_per_batch, number_trajs,
           ...
           find_checkpoint=True, d_policy=None, save_module_dist=False, module=-1):
    if load_model_path:
        d_policy = init_model(envs, load_model_path, find_checkpoint=find_checkpoint, d_policy=d_policy, module=module)
        if d_policy is None:
            return None, None
    ...
    for env in env_dict:
        ...
        if save:
            filename = osp.join(os.path.dirname(load_model_path), 'policy_traj_' + str(number_trajs) + "_" + env + "_M{}".format(module))
...
def traj_1_generator(pi, env, taskid, horizon, stochastic, render, module_dist=False):
    ...
    while True:
        if module_dist:
            taskid = [0 for _ in range(len(taskid))]
