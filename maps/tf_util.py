'''
This script expands on @openai/baselines.gail.tf_utils
'''
...
def flatgrad(loss, var_list, clip_norm=None, clip_min=None, clip_max=None):
    grads = tf.gradients(loss, var_list)
    if clip_norm is not None:
        grads = [tf.clip_by_norm(grad, clip_norm=clip_norm) for grad in grads]
    elif clip_min is not None and clip_max is not None:
        grads = [(tf.clip_by_value(grad, clip_value_min=clip_min, clip_value_max=clip_max))
                     for grad in grads]
